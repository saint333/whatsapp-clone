...
function Main() {
  ...
  useEffect(() => {
    if (socket.current && !socketEvent) {
      socket.current.on("msg-recieve", (data) => {
        console.log("🚀 ~ file: Main.jsx:66 ~ socket.current.on ~ data:", data)
  ...
  useEffect(() => {
    const getMessages = async () => {
      ...
      console.log("🚀 ~ file: Main.jsx:57 ~ getMessages ~ messages:", messages)
      
      dispatch({ type: reducerCases.SET_MESSAGES, messages: messages.messages });
    };
